...
func TestConvertTypedGoalToJSON(t *testing.T) {
	// Create typed goal
	typedGoal := NewGoal("convert-test", "Convert Test", "Test conversion",
		UserInput{Name: "test", Age: 30},
		UserOutput{Message: "hello", Valid: true, Score: 0.8})
	
	// Convert to JSON
	jsonGoal, err := ConvertTypedGoalToJSON(typedGoal)
	if err != nil {
		t.Fatalf("Failed to convert typed goal to JSON: %v", err)
	}
	
	// Verify conversion
	if !jsonGoal.IsSchemaValidated {
		t.Error("Converted goal should be schema validated")
	}
	
	if jsonGoal.UID != typedGoal.UID {
		t.Errorf("UID should be preserved. Expected: %s, Got: %s", typedGoal.UID, jsonGoal.UID)
	}
	
	if jsonGoal.Title != typedGoal.Title {
		t.Errorf("Title should be preserved. Expected: %s, Got: %s", typedGoal.Title, jsonGoal.Title)
	}
	
	// Test converting JSON goal (should return same goal)
	jsonGoal2, err := ConvertTypedGoalToJSON(jsonGoal)
	if err != nil {
		t.Fatalf("Failed to convert JSON goal: %v", err)
	}
	
	if jsonGoal2 != jsonGoal {
		t.Error("Converting JSON goal should return the same goal")
	}
}

func TestGoalReconstruction(t *testing.T) {
	// Test JSON goal reconstruction
	jsonGoal := NewJSONGoal("reconstruct-test", "Reconstruct Test", "Test reconstruction",
		json.RawMessage(`{"name": "test", "age": 25}`),
		json.RawMessage(`{"message": "hello", "valid": true, "score": 0.9}`))
	
	// Clear validators to simulate deserialization
	jsonGoal.InputValidator = nil
	jsonGoal.OutputValidator = nil
	
	// Reconstruct
	if err := jsonGoal.ReconstructValidators(); err != nil {
		t.Fatalf("Failed to reconstruct validators: %v", err)
	}
	
	// Verify validators were reconstructed
	if jsonGoal.InputValidator == nil {
		t.Error("Input validator should be reconstructed")
	}
	
	if jsonGoal.OutputValidator == nil {
		t.Error("Output validator should be reconstructed")
	}
	
	// Test that reconstructed validators work
	validInput := json.RawMessage(`{"name": "Alice", "age": 30}`)
	if err := jsonGoal.InputValidator(validInput); err != nil {
		t.Errorf("Reconstructed input validator should work: %v", err)
	}
	
	validOutput := json.RawMessage(`{"message": "test", "valid": false, "score": 0.7}`)
	if err := jsonGoal.OutputValidator(validOutput); err != nil {
		t.Errorf("Reconstructed output validator should work: %v", err)
	}
}